...
router.get('/api/Usuario/usu/:Apellido1', async(req,res)=>{
    try {
        const nomU1 = db.collection("Usuario");
        const query = nomU1.where("Apellido1", "==", ""+req.params.Apellido1+"");
        const querySnapShot = await query.get();
        const docs = querySnapShot.docs;
        const response = docs.map((doc)=>({
            Id: doc.id,
...
router.get('/api/Usuario/usu/:Apellido2', async(req,res)=>{
    try {
        const nomU2 = db.collection("Usuario");
        const query = nomU2.where("Apellido2", "==", ""+req.params.Apellido2+"");
        const querySnapShot = await query.get();
        const docs = querySnapShot.docs;
        const response = docs.map((doc)=>({
            Id: doc.id,
